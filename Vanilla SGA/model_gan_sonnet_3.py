...
def reset_and_build_graph(
  ...
  apply_vec_disc = list(zip(disc_grads, disc_vars))
  apply_vec_gen = list(zip(gen_grads, gen_vars))
  ...
  with tf.control_dependencies([g for (g, v) in apply_vec_disc]):
    train_op_disc = optimizer.apply_gradients(apply_vec_disc)
  with tf.control_dependencies([g for (g, v) in apply_vec_gen]):
    train_op_gen = optimizer.apply_gradients(apply_vec_gen)

  init = tf.global_variables_initializer()
  
#  return train_op_disc, train_op_gen, x_fake, z, init, disc_loss, gen_loss
  return train_op_disc, train_op_gen, disc_grads, gen_grads, clip_D, x_fake, z, init, disc_loss, gen_loss
