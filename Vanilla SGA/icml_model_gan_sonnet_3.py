...
def jacobian_vec_and_hDiagonal(ys, xs, vs):
...
def jacobian_transpose_vec_and_hDiagonal(ys, xs, vs):
  dydxs = tf.gradients(ys, xs, grad_ys=vs, stop_gradients=xs)
  dydxs = [
      tf.zeros_like(x) if dydx is None else dydx for x, dydx in zip(xs, dydxs)
  ]
  ...
  #hessian_diagonal = [
  #    tf.zeros_like(x) if h_diag is None else h_diag for x, h_diag in zip(xs, hessian_diagonal)
  #]
  ...
  for x, y in zip(xs, ys):
    ...
    #print(x)
  hessian = tf.gradients(ys, xs)# grad_xs = epsilon
  #print("hessian-----------------")
  #print(hessian)
  #print("hessian_diagonal-----------------")
  #print(hessian_diagonal)
  hessianExcluseDiag = list_subtract(hessian, hessian_diagonal)
  dydxs_diagonal=list_multiply(hessianExcluseDiag, vs)
  #dysdx_diagonal=scalar_list_multiply(0.2, dysdx_diagonal)  
  #print("dydxs---------------")
  #print(dydxs)
  #print("dysdx---------------")
  #print(dysdx)
...
#  dy2dx = tf.gradients(dydxs, us)# grad_xs = epsilon

  '''
  hessian_diagonal = []
  for x, dydx, grad in zip(us, dydxs, grad_xs):
    hessian_diagonal.append(tf.gradients(dydx, x, grad_ys = grad)[0])
  hessian_diagonal = [
      tf.zeros_like(x) if h_diag is None else h_diag for x, h_diag in zip(xs, hessian_diagonal)
  ]
  return dysdx,hessian_diagonal
  '''
  hessian_diagonal = []
  for x, dydx in zip(us, dydxs):
    hessian_diagonal.append(tf.gradients(dydx, x)[0])
    #print(x)
...
class SymplecticOptimizer_ICML(tf.train.Optimizer):
  ...
  def apply_gradients(self, grads_and_vars, global_step=None, name=None):
    ...
    #print(grads[1].shape)
    h_v, h_v_diagonal = jacobian_vec_and_hDiagonal(grads, vars_, grads)
    #h_v = jacobian_vec(grads, vars_, grads)
    #h_v_exclude_diagonal = list_subtract(h_v, h_v_diagonal) #A * epislon
    ht_v, ht_v_diagonal = jacobian_transpose_vec_and_hDiagonal(grads, vars_, grads)
    #ht_v = jacobian_transpose_vec(grads, vars_, grads)
    #h_v_diagonal = scalar_list_multiply(0.2,h_v_diagonal)
    #h_v_diagonal = scalar_list_multiply(0.05,h_v_diagonal)
    #at_v = list_divide_scalar(list_subtract(ht_v, h_v), 2.) #A * epislon
    at_v = list_divide_scalar(list_subtract(ht_v_diagonal, h_v_diagonal), 2.) #A * epislon
    #at_v = scalar_list_multiply(0.95, at_v)
    #at_v = list_add(at_v, h_v_diagonal)
    #at_v = list_add(at_v, ht_v_diagonal)
    #at_v = ht_v_diagonal

    #at_v = list_subtract(at_v, ht_v_diagonal) #v2
    #at_v = list_subtract(at_v, h_v_diagonal) #v2
